...
import com.zfh.exception.VoucherException;
...
public class VoucherServiceImpl extends ServiceImpl<VoucherMapper, Voucher> implements IVoucherService {
    ...
    public Boolean addSeckill(SeckillVoucherDto seckillVoucherDto) {
        ...
        //将库存缓存
        stringRedisTemplate.opsForValue().set(RedisKeyConstant.SECKILL_STOCK_KEY + voucher.getId(), seckillVoucherDto.getStock().toString());
     ...
     */
    @Override
    public Boolean updateInfo(VoucherDto voucherDto) {
       if(voucherDto.getId() == null){
           throw  new VoucherException(ExceptionConstant.VOUCHER_NOT_EXIST);
       }
       Voucher voucher = new Voucher();
       BeanUtils.copyProperties(voucherDto, voucher);
    ...
    @Transactional
    @Override
    public Boolean updateSeckill(SeckillVoucherDto seckillVoucherDto) {
       if(seckillVoucherDto.getId() == null){
           throw  new VoucherException(ExceptionConstant.VOUCHER_NOT_EXIST);
       }
       Voucher voucher = new Voucher();
       BeanUtils.copyProperties(seckillVoucherDto, voucher);
       voucher.setUpdateTime(new Date());
       boolean result = updateById(voucher);
      if(! result){
          throw new VoucherException(ExceptionConstant.VOUCHER_NOT_EXIST);
      }
      SeckillVoucher seckillVoucher = new SeckillVoucher();
      BeanUtils .copyProperties(seckillVoucherDto, seckillVoucher);
      seckillVoucher.setUpdateTime(new Date());
      //缓存修改库存
        if(seckillVoucherDto.getStock() != null) {
            stringRedisTemplate.opsForValue().set(RedisKeyConstant.SECKILL_STOCK_KEY + voucher.getId(), seckillVoucherDto.getStock().toString());
        }
      return seckillVoucherService.updateById(seckillVoucher);
    }
